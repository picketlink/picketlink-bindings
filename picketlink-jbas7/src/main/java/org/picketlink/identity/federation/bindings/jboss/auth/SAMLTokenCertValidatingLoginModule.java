 ...
 *   (Role,Membership) 
 *
 * localValidationSecurityDomain: the security domain for the
 *   trust store information (via the JaasSecurityDomain) 
 *
 * cache.invalidation: set it to true if you require invalidation of JBoss 
 *   Auth Cache at SAML Principal expiration. 
 *
 * jboss.security.security_domain: security domain at which
 *   Principal will expire if cache.invalidation is used. 
 *
 * tokenEncodingType: encoding type of SAML token delivered via http request's
 *   header. Possible values are: 
 *
 *   base64 - content encoded as base64. In case of encoding will vary between
 *            base64 and gzip use base64 and LoginModule will detect gzipped
 *            data.  
 ...
 *   samlTokenHttpHeaderRegExGroup to 1. 
 *
